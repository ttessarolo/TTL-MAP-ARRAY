...
class TTLArray {
  ...
  // Rimuove tutti gli elementi scaduti in-place, senza allocare un nuovo array
  _doSweep() {
    const now = Date.now();
    let nextExpiry = null;
    let write = 0;
    for (let read = 0; read < this._arr.length; read++) {
      const item = this._arr[read];
      if (item.expiresAt && item.expiresAt <= now) {
        ...
        this._arr[write++] = item;
      }
    }
    this._arr.length = write;
    return nextExpiry;
  }
